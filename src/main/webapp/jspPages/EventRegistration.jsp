<%-- 
    ...
    Created on : 1 Feb, 2023, 3:41:25 PM
    Author     : yoges
--%>
...
<html>
    <head>
        ...
        <script>
            ...
            function checkRegistration()
            {
                ...
                request.open("POST","<%=application.getContextPath()%>/VerifyUser",true);
                request.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
            ...
            function CheckEventHistroy(event_id,enroll_id,fname,lname,email,mobile,currentdate,curtime)
            {
                try
                {
                    ...
                    request.open("GET","<%=application.getContextPath()%>/CheckRegistration?event="+event_id+"&enroll="+enroll_id,true);
                    request.send();
                }
                catch(err)
                {
    ...
    <body>
        <form name="f1" method="post" enctype="multipart/form-data">
            ...
            <cennter><img src="data:image/*;base64,<c:out value='${requestScope.eventDetail.getBase64Image()}'/>" width="200" height="200" alt="alt"/></cennter>
            <fieldset>
            <legend>Person Info</legend>
            <input type="hidden" name="event_id" value="${requestScope.eventDetail.getEventId()}"/>
            <table>
                <tr>
                    <td><label>Event Name</label></td>
                    <td colspan="2"><input type="text" name="event_name" value="${requestScope.eventDetail.getEventName()}" disabled/></td>
                </tr>
                <tr>
                    <td><label>Event Description</label></td>
                    <td><textarea name="event_description" rows="3" cols="30" disabled>${requestScope.eventDetail.getEventDescription()}</textarea></td>
                </tr>
                <tr>
                    <td><label>Event Date</label></td>
                    <td><input type="date" name="event_date" value="${requestScope.eventDetail.getEventDate()}" disabled/></td>
                </tr>
                <tr>
                    <td><label>Event Fees</label></td>
                    <td><input type="number" name="event_fees" value="${requestScope.eventDetail.getFees()}" disabled/></td>
                </tr>
                <tr>
